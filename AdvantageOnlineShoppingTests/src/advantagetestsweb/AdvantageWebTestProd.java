...
public class AdvantageWebTestProd extends UnitTestClassBase {
    ...
    public void initBeforeTest() throws GeneralLeanFtException {
    	...
    	appModel = new AdvantageAppModel(browser);
    }
    ...
    public void createNewAccountEx(String pUserName, String pPassword, boolean isNegativeTest) throws GeneralLeanFtException, ReportException, InterruptedException
    {
    	...
    	appModel.AdvantageShoppingPage().CreateAccountCountrySelectListBox().select("Israel");
    	...
    	if(!isNegativeTest)
    	{
	    	waitUntilElementExists(appModel.AdvantageShoppingPage().CreateAccountREGISTERValidWebElement());
	    	// Click the Register button
	    	appModel.AdvantageShoppingPage().CreateAccountREGISTERValidWebElement().click();
    ...
    public int getCartProductsNumberFromCartObjectInnerText() throws GeneralLeanFtException
    {
    	...
    	String pattern = appModel.AdvantageShoppingPage().LinkCartIcon().getDescription().getInnerText().toString();
        ...
        m.matches();
        // Extracting the user name from the object's text. It is concatenated to the beginning of the text.
        String productsNunberInCartString = m.group(1).trim();
    ...
    //This test method creates the default user to be used in the tests, if it already does not exist - a user must be in the system:
  	//username - johnhpe1, password - HPEsw123
    @Test
    public void _addMainUserIfNotExists() throws GeneralLeanFtException, ReportException, InterruptedException {
    ...
    public void purchaseTabletTest() throws GeneralLeanFtException, InterruptedException {
    	...
    	// Select an item to purchase and add it to the cart
    	try{
    	selectItemToPurchase(appModel.AdvantageShoppingPage().TabletsImgWebElement(),appModel.AdvantageShoppingPage().HPEliteTablet1011Image());
    	}
    	catch(Exception e)
    	{
    		e.printStackTrace();
    ...
    public void purchaseLaptopTest() throws GeneralLeanFtException, InterruptedException {
    	...
    	// Select an item to purchase and add it to the cart
    	try{
    	selectItemToPurchase(appModel.AdvantageShoppingPage().LAPTOPSShopNowWebElement(),appModel.AdvantageShoppingPage().HPChromebookEnergyStarImage());
    	}
    	catch(Exception e)
    	{
    		e.printStackTrace();
    ...
    public void purchaseMouseTest() throws GeneralLeanFtException, InterruptedException {
    	...
    	// Select an item to purchase and add it to the cart
    	try{
    	selectItemToPurchase(appModel.AdvantageShoppingPage().MICEShopNowWebElement(),appModel.AdvantageShoppingPage().HPUsbOpticalMouseImage());
    	}
    	catch(Exception e)
    	{
    		e.printStackTrace();
    ...
    public void purchaseHeadphonesTest() throws GeneralLeanFtException, InterruptedException {
    	...
    	// Select an item to purchase and add it to the cart
    	try{
    	selectItemToPurchase(appModel.AdvantageShoppingPage().HEADPHONESShopNowWebElement(),appModel.AdvantageShoppingPage().HPBeatsStudioHeadphonesImage());
    	}
    	catch(Exception e)
    	{
    		e.printStackTrace();
    ...
    public void contactUsTest() throws GeneralLeanFtException, InterruptedException {
    	...
    	appModel.AdvantageShoppingPage().SelectProductsContactUsListBox().select("HP Roar Mini Wireless Speaker");
    	appModel.AdvantageShoppingPage().EmailContactUsWebElement().setValue("john@hpe.com");
    	appModel.AdvantageShoppingPage().WebEditEditFieldContactUsSubject().setValue("Thank you");
    	
    	// Submit the form by sending it
    	appModel.AdvantageShoppingPage().SENDContactUsWebElement().click();
    ...
    public void verifyUserLinks() throws GeneralLeanFtException, InterruptedException
    {
    	...
    	Verify.isTrue(!isSignedIn(),"Verification - Verify User Links","Verify that the user links navigations work - Sign Out.");
